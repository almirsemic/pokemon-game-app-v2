...
const NewFighter = (props: typeProps) => {
  ...
  function handlePokemons(){
    if(props.pokemon === "pokemonOne"){
      dispatch(pokemonOneState({pokemonOneNumber: Math.floor(Math.random() * 100) + 1}))
    }else if(props.pokemon === "pokemonTwo"){
      dispatch(pokemonTwoState({pokemonTwoNumber: Math.floor(Math.random() * 100) + 1}))
    }
  }
  return (
    <div className="my-5">
      <div className={Styles.box}>
        <div className={Styles.btn}>
          <button className="fw-bold" onClick={() => handlePokemons()}>NEW</button>
        </div>
      </div>
    </div>
  )
